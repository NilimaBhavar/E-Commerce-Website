...
export function ProductCard({ product }) {
  ...
  return <Link href={`/products/${product.id}`} data-testid={`card-product-${product.id}`}>
    ...
    /* Discount badge */
        ...
        {product.discount && product.discount > 0 && <Badge className="absolute top-2 left-2 z-10 bg-primary text-primary-foreground text-xs">
  ...
  />
    ...
    /* Content */
        ...
        <div className="p-3 flex flex-col flex-1">
          {product.categoryName && <p className="text-xs text-muted-foreground mb-1 truncate">{product.categoryName}</p>}
    ...
    /* Price */
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            {product.originalPrice && product.originalPrice > product.price && <span className="text-xs text-muted-foreground line-through">₹{product.originalPrice}</span>}
          </div>

          {
    /* Stock */
  }
          {product.stock === 0 && <p className="text-xs text-destructive mb-2">Out of stock</p>}
