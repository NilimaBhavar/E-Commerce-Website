...
import { useGetFeaturedProducts, useGetOfferProducts, useListCategories } from "@/hooks/useApi";
...
export function HomePage() {
  ...
  return <div>
  ...
  ].map(({ icon: Icon, text }) => <div key={text} className="flex items-center justify-center gap-2 py-1">
    ...
    /* Categories */
      ...
      <section className="container mx-auto px-4 py-12">
        ...
        {catsLoading ? <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            {categories?.map((cat) => <Link key={cat.id} href={`/products?categoryId=${cat.id}`} data-testid={`card-category-${cat.id}`}>
                <div className="group relative rounded-xl overflow-hidden bg-accent hover:shadow-md transition-all duration-200 cursor-pointer border border-border">
                  <div className="aspect-square overflow-hidden">
                    {cat.imageUrl && <img src={cat.imageUrl} alt={cat.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />}
                  </div>
    ...
    /* Featured Products */
      ...
      <section className="bg-muted/40 py-12">
        <div className="container mx-auto px-4">
          ...
          {featLoading ? <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    ...
    /* Offers */
      ...
      <section className="container mx-auto px-4 py-12">
        ...
        {offersLoading ? <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-64 rounded-xl" />)}
          </div> : <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
