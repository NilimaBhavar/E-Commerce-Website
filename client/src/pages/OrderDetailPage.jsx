...
export function OrderDetailPage() {
  ...
  return <div className="container mx-auto px-4 py-10 max-w-3xl">
    ...
    /* Status timeline */
      ...
      {order.status !== "cancelled" && <Card className="mb-6">
          <CardContent className="pt-6">
            <div className="flex items-center">
              {statusSteps.map((step, i) => <div key={step} className="flex items-center flex-1 last:flex-none">
                  ...
                  {i < statusSteps.length - 1 && <div className={`flex-1 h-0.5 ${i < currentStep ? "bg-primary" : "bg-muted"}`} />}
                </div>)}
            </div>
          </CardContent>
        </Card>}
    ...
    /* Items */
      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-4">
          {order.items?.map((item) => <div key={item.id} className="flex gap-3 items-center">
