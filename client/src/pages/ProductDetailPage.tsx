...
export function ProductDetailPage() {
  ...
  const { data: reviews } = useListReviews(numId, {
    query: { enabled: !!numId, queryKey: getListReviewsQueryKey(numId) }
  });
  ...
  const isWishlisted = wishlist?.some((w) => w.productId === numId) ?? false;

  function handleAddToCart() {
    if (!isAuthenticated) { toast({ title: "Please login to add to cart", variant: "destructive" }); return; }
    addToCart.mutate({ data: { productId: numId, quantity } }, {
      onSuccess: () => { queryClient.invalidateQueries({ queryKey: getGetCartQueryKey() }); toast({ title: "Added to cart!", description: `${product?.name} (x${quantity})` }); },
      onError: () => toast({ title: "Failed to add to cart", variant: "destructive" }),
    });
  }

  function handleWishlist() {
    if (!isAuthenticated) { toast({ title: "Please login to save items", variant: "destructive" }); return; }
    if (isWishlisted) {
      removeFromWishlist.mutate({ productId: numId }, {
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: getGetWishlistQueryKey() }); toast({ title: "Removed from wishlist" }); },
      });
    } else {
      addToWishlist.mutate({ productId: numId }, {
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: getGetWishlistQueryKey() }); toast({ title: "Saved to wishlist!" }); },
      });
    }
  }
  ...
  function handleReview(e: React.FormEvent) {
    e.preventDefault();
    if (!isAuthenticated) { toast({ title: "Please login to write a review", variant: "destructive" }); return; }
    createReview.mutate({ productId: numId, data: { rating: reviewRating, comment: reviewComment } }, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListReviewsQueryKey(numId) });
        queryClient.invalidateQueries({ queryKey: getGetProductQueryKey(numId) });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
        ...
        <div className="relative">
          {product.discount && product.discount > 0 && (
            <Badge className="absolute top-3 left-3 z-10 bg-primary text-primary-foreground">{product.discount}% OFF</Badge>
          )}
        ...
        <div>
          ...
          <div className="flex items-baseline gap-3 mb-4">
            ...
            {product.discount && product.discount > 0 && (
              <span className="text-sm font-semibold text-primary">Save ₹{(product.originalPrice! - product.price).toFixed(0)}</span>
            )}
          </div>

          <div className="mb-4">
            {product.stock > 0 ? (
              <p className="text-secondary font-medium text-sm flex items-center gap-1"><Package className="h-4 w-4" /> In Stock ({product.stock} available)</p>
            ) : (
