...
export function ProductListPage() {
  ...
  const { data: result, isLoading } = useListProducts({
    ...
    inStock: inStockOnly ? true : void 0
  });
  useEffect(() => {
  ...
  const FiltersPanel = () => <div className="space-y-6">
      <div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <input type="radio" id="cat-all" name="category" value="" checked={categoryId === ""} onChange={() => {
    setCategoryId("");
    setPage(1);
  }} className="accent-primary" />
            <label htmlFor="cat-all" className="text-sm cursor-pointer">All Categories</label>
          </div>
          {categories?.map((cat) => <div key={cat.id} className="flex items-center gap-2">
              <input type="radio" id={`cat-${cat.id}`} name="category" value={String(cat.id)} checked={categoryId === String(cat.id)} onChange={() => {
    ...
    setPage(1);
      ...
      <div>
        ...
        <Slider min={0} max={2e3} step={50} value={priceRange} onValueChange={(v) => {
    setPriceRange(v);
    setPage(1);
  }} className="mb-2" />
      ...
      <div className="flex items-center gap-2">
        <Checkbox id="instock" checked={inStockOnly} onCheckedChange={(v) => {
    setInStockOnly(!!v);
    setPage(1);
  }} />
  ...
  />
    ...
    /* Mobile filter trigger */
        ...
        {result?.total && <span className="text-sm text-muted-foreground ml-auto">{result.total} products</span>}
      </div>
    ...
    /* Grid */
        ...
        <div className="flex-1">
          {isLoading ? <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {Array.from({ length: 12 }).map((_, i) => <Skeleton key={i} className="h-64 rounded-xl" />)}
            </div> : result?.products?.length ? <>
    ...
    /* Pagination */
              ...
              {result.totalPages > 1 && <div className="flex items-center justify-center gap-3 mt-8">
                  <Button variant="outline" size="sm" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}>
                    <ChevronLeft className="h-4 w-4" />
                  </Button>
                  {Array.from({ length: result.totalPages }, (_, i) => i + 1).filter((p) => Math.abs(p - page) <= 2).map((p) => <Button key={p} variant={p === page ? "default" : "outline"} size="sm" onClick={() => setPage(p)}>
                        {p}
                      </Button>)}
